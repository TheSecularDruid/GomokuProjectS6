...
int play_move (struct col_move_t *move_done, struct bitboard *board, size_t size){ //plays a move
...
int color_is_winning(__uint128_t colored_board, size_t size, size_t winning_threshold){ //winning_threshold is 5 in the classical rules
    ...
    for (size_t i=0; i<size-winning_threshold+1; i++) {
        for (size_t j=0; j<size-winning_threshold+1; j++) {
            ...
            || (  ( (i==size-winning_threshold) || (j==0) ) //downward diagonal touches right or bottom border
                && ( pattern_is_matching(pattern_diag_down, colored_board) && pattern_is_matching(pattern_diag_up>>(size+1), colored_board)==0 )  )
            || ( pattern_is_matching(pattern_diag_down, colored_board) && pattern_is_matching(pattern_diag_up>>(size+1), colored_board)==0  //downward diagonal doesn't touch any corner: overlines are possibles both ways
  ...
  /*int main_test(){
    __uint128_t x= 1+64+64*64+64*64*64+64*64*64*64;
    __uint128_t y= 3+64+64*64+64*64*64+64*64*64*64;
    //x+=64*64*64*64*64;
    printf("%llu\n", bitwise_and(x, y) );
    printf("%d\n", pattern_is_matching( x,y));
    printf ("%d\n" ,color_is_winning(x , 6, 5));
        return 0;
  }*/
